...
import concierge.org.conciergewallet.ui.contacts_activity.ContactsActivity;
import concierge.org.conciergewallet.ui.donate.DonateActivity;
...
public class BaseDrawerActivity extends PivxActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = conciergeApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = conciergeModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (conciergeModule.getChainHeight()+10>peerHeight) {
            ...
            if (conciergeModule.isAnyPeerConnected()) {
                long peerHeight = conciergeModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (conciergeModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = conciergeModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (conciergeModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
