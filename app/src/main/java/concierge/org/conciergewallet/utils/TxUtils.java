    ...
    public static String getAddressOrContact(ConciergeModule conciergeModule, TransactionWrapper data) {
        ...
        if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
            ...
            if (addressLabel !=null) {
                ...
                try {
                    text = data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(conciergeModule.getConf().getNetworkParams(), true).toBase58();
                }catch (ScriptException e){
                    text = data.getTransaction().getOutput(1).getScriptPubKey().getToAddress(conciergeModule.getConf().getNetworkParams(),true).toBase58();
                }
            }
        }else {
            text = "Error";
