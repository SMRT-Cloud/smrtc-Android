package smartcloud.cash.smartcloudwallet.service;
...
import org.smartcloudj.core.Block;
import org.smartcloudj.core.Coin;
import org.smartcloudj.core.FilteredBlock;
import org.smartcloudj.core.Peer;
import org.smartcloudj.core.Transaction;
import org.smartcloudj.core.TransactionConfidence;
import org.smartcloudj.core.listeners.AbstractPeerDataEventListener;
...
import smartcloud.cash.smartcloudwallet.module.store.SnappyBlockchainStore;
import smartcloud.cash.smartcloudwallet.rate.CoinMarketCapApiClient;
import smartcloud.cash.smartcloudwallet.rate.RequestSmartcloudRateException;
...
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.ACTION_NOTIFICATION;
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.ACTION_RESET_BLOCKCHAIN;
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
...
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static smartcloud.cash.smartcloudwallet.service.IntentsConstants.NOT_COINS_RECEIVED;

/**
 * Created by Telostia on 6/12/17.
 */

public class SmartcloudWalletService extends Service {

    private Logger log = LoggerFactory.getLogger(SmartcloudWalletService.class);

    private SmartcloudApplication smartcloudApplication;
    private SmartcloudModuleImp module;
    ...
    public class SmartcloudBinder extends Binder {
        public SmartcloudWalletService getService() {
            return SmartcloudWalletService.this;
        }
    }
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (SmartcloudContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    smartcloudApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
    ...
    private class RunnableBlockChecker implements Runnable {
        ...
        @Override
        public void run() {
            org.smartcloudj.core.Context.propagate(SmartcloudContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.smartcloudj.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), SmartcloudWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(SmartcloudWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                    mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            .setContentTitle("SMRTC received!")
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(SmartcloudWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("Smartcloud service started");
            // Android stuff
            ...
            module = (SmartcloudModuleImp) smartcloudApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to pivtrum node
            /*pivtrumPeergroup = new PivtrumPeergroup(smartcloudApplication.getNetworkConf());
            ...
            String filename = SmartcloudContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file, filename).exists();
            blockchainStore = new SnappyBlockchainStore(SmartcloudContext.CONTEXT, file, filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("Smartcloud service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void tryScheduleService() {
        ...
        if (!isSchedule) {
            ...
            Intent intent = new Intent(this, SmartcloudWalletService.class);
    ...
    private void requestRateCoin() {
        final AppConf appConf = smartcloudApplication.getAppConf();
        SmartcloudRate smartcloudRate = module.getRate(appConf.getSelectedRateCoin());
        if (smartcloudRate == null || smartcloudRate.getTimestamp() + SmartcloudContext.RATE_UPDATE_TIME < System.currentTimeMillis()) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        CoinMarketCapApiClient.SmartcloudMarket smartcloudMarket = c.getSmartcloudPxrice();
                        SmartcloudRate smartcloudRate = new SmartcloudRate("USD", smartcloudMarket.priceUsd, System.currentTimeMillis());
                        module.saveRate(smartcloudRate);
                        final SmartcloudRate smartcloudBtcRate = new SmartcloudRate("BTC", smartcloudMarket.priceBtc, System.currentTimeMillis());
                        module.saveRate(smartcloudBtcRate);

                        // Get the rest of the rates:
                        List<SmartcloudRate> rates = new CoinMarketCapApiClient.BitPayApi().getRates(new CoinMarketCapApiClient.BitPayApi.RatesConvertor<SmartcloudRate>() {
                            @Override
                            public SmartcloudRate convertRate(String code, String name, BigDecimal bitcoinRate) {
                                BigDecimal rate = bitcoinRate.multiply(smartcloudBtcRate.getRate());
                                return new SmartcloudRate(code, rate, System.currentTimeMillis());
                            }
                        });

                        for (SmartcloudRate rate : rates) {
                            module.saveRate(rate);
                        }

                    } catch (RequestSmartcloudRateException e) {
                        e.printStackTrace();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
