package smartcloud.cash.smartcloudwallet.rate.db;
...
import smartcloud.cash.smartcloudwallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<SmartcloudRate> implements RateDbDao<SmartcloudRate> {
    ...
    @Override
    protected ContentValues buildContent(SmartcloudRate obj) {
    ...
    @Override
    protected SmartcloudRate buildFrom(Cursor cursor) {
        ...
        return new SmartcloudRate(coin, value, timestap);
    }
    ...
    public void insertOrUpdateIfExist(SmartcloudRate smartcloudRate) {
        if (getRate(smartcloudRate.getCode()) == null) {
            insert(smartcloudRate);
        } else {
            updateByKey(KEY_COIN, smartcloudRate.getCode(), smartcloudRate);
        }
    }
}
