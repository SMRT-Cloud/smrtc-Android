package smartcloud.cash.smartcloudwallet.ui.pincode_activity;
...
import smartcloud.cash.smartcloudwallet.ui.backup_mnemonic_activity.MnemonicActivity;
import smartcloud.cash.smartcloudwallet.ui.base.BaseActivity;
import smartcloud.cash.smartcloudwallet.ui.settings_pincode_activity.KeyboardFragment;
import smartcloud.cash.smartcloudwallet.ui.start_activity.StartActivity;

import static smartcloud.cash.smartcloudwallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (smartcloudApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (smartcloudApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            smartcloudApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        smartcloudApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(smartcloudApplication.getAppConf().getPincode().equals(pincode)){
