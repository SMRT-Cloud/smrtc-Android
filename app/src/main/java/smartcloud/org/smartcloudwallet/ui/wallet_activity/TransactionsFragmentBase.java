...
import org.smartcloudj.utils.MonetaryFormat;
...
import smartcloud.cash.smartcloudwallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static smartcloud.cash.smartcloudwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static smartcloud.cash.smartcloudwallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static smartcloud.cash.smartcloudwallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = smartcloudModule.listTx();
        ...
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (smartcloudRate!=null) {
                    localCurrency = smartcloudApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * smartcloudRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                ...
                holder.title.setText(getAddressOrContact(smartcloudModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(smartcloudModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(smartcloudModule.getConf().getNetworkParams()).toBase58());
                }*/
