...
import smartcloud.cash.smartcloudwallet.ui.contacts_activity.ContactsActivity;
import smartcloud.cash.smartcloudwallet.ui.donate.DonateActivity;
import smartcloud.cash.smartcloudwallet.ui.settings_activity.SettingsActivity;
import smartcloud.cash.smartcloudwallet.ui.wallet_activity.WalletActivity;

import static smartcloud.cash.smartcloudwallet.module.SmartcloudContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends SmartcloudActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState() {
        ...
        long lastBlockTime = smartcloudApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime + OUT_OF_SYNC_TIME > now) {
            // check if i'm syncing or i'm synched
            long peerHeight = smartcloudModule.getConnectedPeerHeight();
            if (peerHeight != -1) {
                if (smartcloudModule.getChainHeight() + 10 > peerHeight) {
            ...
            if (smartcloudModule.isAnyPeerConnected()) {
                long peerHeight = smartcloudModule.getConnectedPeerHeight();
                if (peerHeight != -1) {
                    if (smartcloudModule.getChainHeight() + 10 > peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = smartcloudModule.getConnectedPeerHeight();
        if (nodeHeight > 0) {
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (smartcloudModule.getChainHeight() * 100) / nodeHeight;
        }
        return -1;
    }
