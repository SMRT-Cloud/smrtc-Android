package aquila.org.aquilawallet.service;
...
import aquila.org.aquilawallet.module.AquilaContext;
import global.AquilaModuleImp;
import aquila.org.aquilawallet.module.store.SnappyBlockchainStore;
import aquila.org.aquilawallet.rate.CoinMarketCapApiClient;
import aquila.org.aquilawallet.rate.RequestAquilaRateException;
...
import static aquila.org.aquilawallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static aquila.org.aquilawallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static aquila.org.aquilawallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
import static aquila.org.aquilawallet.service.IntentsConstants.ACTION_NOTIFICATION;
import static aquila.org.aquilawallet.service.IntentsConstants.ACTION_RESET_BLOCKCHAIN;
import static aquila.org.aquilawallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
import static aquila.org.aquilawallet.service.IntentsConstants.DATA_TRANSACTION_HASH;
import static aquila.org.aquilawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_BLOCKCHAIN_STATE;
import static aquila.org.aquilawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_ON_COIN_RECEIVED;
import static aquila.org.aquilawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_PEER_CONNECTED;
import static aquila.org.aquilawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_TYPE;
import static aquila.org.aquilawallet.service.IntentsConstants.INTENT_EXTRA_BLOCKCHAIN_STATE;
import static aquila.org.aquilawallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static aquila.org.aquilawallet.service.IntentsConstants.NOT_COINS_RECEIVED;

/**
 * Created by MotoAcidic on 6/12/17.
 */

public class AquilaWalletService extends Service{

    private Logger log = LoggerFactory.getLogger(AquilaWalletService.class);

    private AquilaApplication aquilaApplication;
    private AquilaModuleImp module;
    ...
    public class AquilaBinder extends Binder {
        public AquilaWalletService getService() {
            return AquilaWalletService.this;
        }
    }
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (AquilaContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    aquilaApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.aquilaj.core.Context.propagate(AquilaContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.aquilaj.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), AquilaWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(AquilaWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                    mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            .setContentTitle("AQX received!")
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(AquilaWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("Aquila service started");
            // Android stuff
            ...
            // Aquila
            aquilaApplication = AquilaApplication.getInstance();
            module = (AquilaModuleImp) aquilaApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to pivtrum node
            /*pivtrumPeergroup = new PivtrumPeergroup(aquilaApplication.getNetworkConf());
            ...
            String filename = AquilaContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file,filename).exists();
            blockchainStore = new SnappyBlockchainStore(AquilaContext.CONTEXT,file,filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("Aquila service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void tryScheduleService() {
        ...
        if (!isSchedule){
            ...
            Intent intent = new Intent(this, AquilaWalletService.class);
    ...
    private void requestRateCoin(){
        final AppConf appConf = aquilaApplication.getAppConf();
        AquilaRate aquilaRate = module.getRate(appConf.getSelectedRateCoin());
        if (aquilaRate == null || aquilaRate.getTimestamp() + AquilaContext.RATE_UPDATE_TIME < System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        CoinMarketCapApiClient.AquilaMarket aquilaMarket = c.getAquilaPxrice();
                        AquilaRate aquilaRate = new AquilaRate("USD",aquilaMarket.priceUsd,System.currentTimeMillis());
                        module.saveRate(aquilaRate);
                        final AquilaRate aquilaBtcRate = new AquilaRate("BTC",aquilaMarket.priceBtc,System.currentTimeMillis());
                        module.saveRate(aquilaBtcRate);

                        // Get the rest of the rates:
                        List<AquilaRate> rates = new CoinMarketCapApiClient.BitPayApi().getRates(new CoinMarketCapApiClient.BitPayApi.RatesConvertor<AquilaRate>() {
                            @Override
                            public AquilaRate convertRate(String code, String name, BigDecimal bitcoinRate) {
                                BigDecimal rate = bitcoinRate.multiply(aquilaBtcRate.getRate());
                                return new AquilaRate(code,rate,System.currentTimeMillis());
                            }
                        });

                        for (AquilaRate rate : rates) {
                            module.saveRate(rate);
                        }

                    } catch (RequestAquilaRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
