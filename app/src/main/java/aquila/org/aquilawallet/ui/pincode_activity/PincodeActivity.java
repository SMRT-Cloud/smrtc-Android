package aquila.org.aquilawallet.ui.pincode_activity;
...
import aquila.org.aquilawallet.ui.backup_mnemonic_activity.MnemonicActivity;
import aquila.org.aquilawallet.ui.base.BaseActivity;
import aquila.org.aquilawallet.ui.settings_pincode_activity.KeyboardFragment;
import aquila.org.aquilawallet.ui.start_activity.StartActivity;

import static aquila.org.aquilawallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (aquilaApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (aquilaApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            aquilaApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        aquilaApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(aquilaApplication.getAppConf().getPincode().equals(pincode)){
