package aquila.org.aquilawallet.ui.base;
...
import aquila.org.aquilawallet.ui.contacts_activity.ContactsActivity;
import aquila.org.aquilawallet.ui.donate.DonateActivity;
import aquila.org.aquilawallet.ui.settings_activity.SettingsActivity;
import aquila.org.aquilawallet.ui.wallet_activity.WalletActivity;

import static aquila.org.aquilawallet.module.AquilaContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends AquilaActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = aquilaApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = aquilaModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (aquilaModule.getChainHeight()+10>peerHeight) {
            ...
            if (aquilaModule.isAnyPeerConnected()) {
                long peerHeight = aquilaModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (aquilaModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = aquilaModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (aquilaModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
