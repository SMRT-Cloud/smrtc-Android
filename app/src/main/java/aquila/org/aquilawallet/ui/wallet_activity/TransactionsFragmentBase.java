...
import aquila.org.aquilawallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static aquila.org.aquilawallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static aquila.org.aquilawallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static aquila.org.aquilawallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = aquilaModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (aquilaRate!=null) {
                    localCurrency = aquilaApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * aquilaRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + aquilaRate.getCode();
                ...
                holder.title.setText(getAddressOrContact(aquilaModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(aquilaModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(aquilaModule.getConf().getNetworkParams()).toBase58());
                }*/
