package aquila.org.aquilawallet.rate.db;
...
import aquila.org.aquilawallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<AquilaRate> implements RateDbDao<AquilaRate> {
    ...
    @Override
    protected ContentValues buildContent(AquilaRate obj) {
    ...
    @Override
    protected AquilaRate buildFrom(Cursor cursor) {
        ...
        return new AquilaRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(AquilaRate aquilaRate) {
        if (getRate(aquilaRate.getCode())==null){
            insert(aquilaRate);
        }else {
            updateByKey(KEY_COIN,aquilaRate.getCode(),aquilaRate);
        }
    }
}
